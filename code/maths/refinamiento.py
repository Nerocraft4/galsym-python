...
# Inicializamos matriz DF y vector F a cero
    ...
    xnorm2 = 0
    for i in range(0,ne):
        xnorm2 = xnorm2+f[i]*f[i] #TODO optim amb numpy, també x llegibilitat
    xnorm2s = xnorm2 #TODO probablement redundant

    # Resolvemos el sistema DF * AUX = F mediante el metodo SVD
    u,d,v = np.linalg.svd(df) #TODO potser call afegir , full_matrices=False
    ...
    # Refs https://help.perforce.com/pv-wave/2023.1/PVWAVE_Online_Help/Foundation/svbksb.html
    # https://github.com/nimeshpatel/holographyReductionPython/blob/main/svbksb1.f
    aux = 0 #TODO svbksb(df,w,v,ne,nc,f)

    #xkk(1:nc) = aux(1:nc) + xkk(1:nc)
    # Calculamos la norma entre dos iteraciones
    xnorms = np.linalg.norm(aux)
