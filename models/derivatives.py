...
from ..maths.elint import elint
from ..maths.xlmbd import xlmbd
import numpy as np

def der1bar(barra,parsb,x,y,z,omega):
...
def der1disk(barra,parsb,x,y,z,omega):
    pxd,pyd,pzd = [0,0,0]
    return [pxd,pyd,pzd]

def der1bulge(barra,parsb,x,y,z,omega):
    pxbl,pybl,pzbl = [0,0,0]
    return [pxbl,pybl,pzbl]

def der1halo(barra,parsb,x,y,z,omega):
    pxh,pyh,pzh = [0,0,0]
    return [pxh,pyh,pzh]

def der2bar(barra,parsb,x,y,z,omega):
    pxxb,pyyb,pzzb,pxyb,pxzb,pyzb = [0,0,0,0,0,0]
    return [pxxb,pyyb,pzzb,pxyb,pxzb,pyzb]

def der2disk(barra,parsb,x,y,z,omega):
    pxxd,pyyd,pzzd,pxyd,pxzd,pyzd = [0,0,0,0,0,0]
    return [pxxd,pyyd,pzzd,pxyd,pxzd,pyzd]

def der2bulge(barra,parsb,x,y,z,omega):
    pxxbl,pyybl,pzzbl,pxybl,pxzbl,pyzbl = [0,0,0,0,0,0]
    return [pxxbl,pyybl,pzzbl,pxybl,pxzbl,pyzbl]

def der2halo(barra,parsb,x,y,z,omega):
